...
namespace MassBattle.Logic.Armies
{
    public class ArmyProvider : IArmyProvider
    {
        ...
        {
            return _armiesData.FirstOrDefault(army => army.ArmySetup.Id == armyId);
        }
        ...
        private void FillUpEnemiesForRegisteredArmies()
        {
            for (int i = 0; i < _armiesData.Count; i++)
            {
                ...
                List<ArmyData> enemies = FindEnemiesForArmyId(currentArmy.ArmySetup.Id);
        ...
        {
            return _armiesData.FindAll(army => army.ArmySetup.Id != armyId);
        }
