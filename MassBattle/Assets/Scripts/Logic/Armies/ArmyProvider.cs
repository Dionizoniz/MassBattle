...
namespace MassBattle.Logic.Armies
{
    public class ArmyProvider : IArmyProvider
    {
        ...
        {
            return _armiesData.FirstOrDefault(army => army.InitialArmyData.Id == armyId);
        }
        ...
        private void FillUpEnemiesForRegisteredArmies()
        {
            for (int i = 0; i < _armiesData.Count; i++)
            {
                ...
                List<ArmyData> enemies = FindEnemiesForArmyId(currentArmy.InitialArmyData.Id);
        ...
        {
            return _armiesData.FindAll(army => army.InitialArmyData.Id != armyId);
        }
