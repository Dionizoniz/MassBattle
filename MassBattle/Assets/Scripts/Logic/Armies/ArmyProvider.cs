...
namespace MassBattle.Logic.Armies
{
    public class ArmyProvider : IArmyProvider
    {
        ...
        {
            return _armiesData.FirstOrDefault(army => army.DescriptorId == armyId);
        }
        ...
        private void FillUpEnemiesForRegisteredArmies()
        {
            for (int i = 0; i < _armiesData.Count; i++)
            {
                ...
                List<ArmyData> enemies = FindEnemiesForArmyId(currentArmy.DescriptorId);
        ...
        {
            return _armiesData.FindAll(army => army.DescriptorId != armyId);
        }
